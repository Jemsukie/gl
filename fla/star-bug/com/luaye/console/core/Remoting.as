package com.luaye.console.core
{
   ...
   public class Remoting extends EventDispatcher
   {
      ...
      public function update(mspf:Number, sFR:Number = NaN) : void
      {
         ...
         if(sFR)
         {
            if((frames = Math.floor(mspf / (1000 / sFR))) > 25)
            {
               frames = 25;
            }
            while(frames > 1)
      ...
      {
         var target:String = Console.REMOTING_CONN_NAME + (this._isRemote ? "C" : "R");
      ...
      public function set remoting(newV:Boolean) : void
      {
         ...
         if(newV)
         {
            ...
            this._remoteLinesQueue = [];
            this.startSharedConnection();
            this._sharedConnection.addEventListener("securityError",this.onRemotingSecurityError);
            try
            {
               this._sharedConnection.connect(Console.REMOTING_CONN_NAME + "C");
      ...
      public function set isRemote(newV:Boolean) : void
      {
         ...
         if(newV)
         {
            this._isRemoting = false;
            this.startSharedConnection();
            this._sharedConnection.addEventListener("status",this.onRemoteStatus);
            this._sharedConnection.addEventListener("securityError",this.onRemotingSecurityError);
            try
            {
               this._sharedConnection.connect(Console.REMOTING_CONN_NAME + "R");
               this._master.report("<b>Remote started.</b> " + this.getInfo(),-1);
               sdt = Security.sandboxType;
               if(sdt == "localWithFile" || sdt == "localWithNetwork")
               {
