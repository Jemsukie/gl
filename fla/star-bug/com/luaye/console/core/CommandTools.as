   ...
   public class CommandTools
   {
      ...
      public function inspect(obj:Object, viewAll:Boolean = true) : void
      {
         ...
         for each(methodX in nodes)
         {
            ...
            (arr = self == methodX.@declaredBy ? props : props2).push(str);
         }
         ...
         {
            str = (str = accessorX.parent().name() == "factory" ? "" : staticPrefix) + ((accessorX.@access == "readonly" ? "<i>" + accessorX.@name + "</i>" : accessorX.@name) + "<p0>(" + accessorX.@type + ")</p0>");
            (arr = self == accessorX.@declaredBy ? props : props2).push(str);
         }
         ...
         {
            props = [];
            clen = (mc = obj as DisplayObjectContainer).numChildren;
            for(ci = 0; ci < clen; )
            {
      ...
      public function map(base:DisplayObjectContainer, maxStep:uint = 0) : void
      {
         ...
         var basestr:String = this._mapBaseIndex + "|";
         var list:Array = [];
         var index:int = 0;
         list.push(base);
         while(index < list.length)
         {
            if((mcDO = list[index]) is DisplayObjectContainer)
            {
               numC = (mc = mcDO as DisplayObjectContainer).numChildren;
               for(i = 0; i < numC; )
               {
         ...
         for(X in list)
         {
            ...
            if(lastmcDO)
            {
               ...
               else
               {
                  while(lastmcDO)
                  {
                     if((lastmcDO = lastmcDO.parent) is DisplayObjectContainer)
                     {
                        if(steps > 0)
                        {
                           indexes.pop();
            ...
            {
               wasHiding = false;
               n = "<a href=\'event:clip_" + basestr + indexes.join("|") + "\'>" + mcDO.name + "</a>";
               ...
               this.report(str,mcDO is DisplayObjectContainer ? 5 : 2);
            }
            else if(!wasHiding)
            {
               wasHiding = true;
      ...
      public function reMap(path:String, mc:DisplayObjectContainer) : DisplayObject
      {
         ...
         var first:String;
         var pathArr:Array;
         if((first = (pathArr = path.split("|")).shift()) != "0")
         {
         ...
         try
         {
            for each(nn in pathArr)
            {
               ...
               if(!((child = mc.getChildByName(nn)) is DisplayObjectContainer))
               {
                  break;
               }
