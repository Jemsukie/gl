package com.luaye.console
{
   ...
   public class Console extends Sprite
   {
      ...
      public function Console(pass:String = "", uiset:int = 1)
      {
         ...
         this.report("<b>Console v2.31" + "" + ", Happy bug fixing! This text last updated 2023-08-29 17:08. Password = " + pass + "</b>",-2);
      ...
      public static function get remoteIsRunning() : Boolean
      {
         ...
         try
         {
            ...
            sCon.connect(REMOTING_CONN_NAME + "R");
         }
         catch(error:Error)
         {
            return true;
      ...
      private function stageAddedHandle(e:Event = null) : void
      {
         ...
         addEventListener("enterFrame",this._onEnterFrame,false,0,true);
         parent.addEventListener("added",this.onParentDisplayAdded,false,0,true);
         stage.addEventListener("mouseLeave",this.onStageMouseLeave,false,0,true);
         stage.addEventListener("keyDown",this.keyUpHandler,false,0,true);
      }
      
      private function stageRemovedHandle(e:Event = null) : void
      {
         removeEventListener("enterFrame",this._onEnterFrame);
         parent.removeEventListener("added",this.onParentDisplayAdded);
         stage.removeEventListener("mouseLeave",this.onStageMouseLeave);
         stage.removeEventListener("keyDown",this.keyUpHandler);
      }
      ...
      public function set channelsPanel(b:Boolean) : void
      {
         ...
         if(b)
         {
            chPanel = this.panels.getPanel("channelsPanel") as ChannelsPanel;
      ...
      private function onRollerCaptureKey() : void
      {
         ...
         {
            this.report("Display Roller Capture:" + RollerPanel(this.panels.getPanel("rollerPanel")).capture(),-1);
         }
      }
      ...
      private function _onEnterFrame(e:Event) : void
      {
         ...
         if(this._needToMoveTop && this.alwaysOnTop && visible && parent)
         {
      ...
      private function remoteLogSend(obj:Array) : void
      {
         ...
         if(remoteMSPFs = obj[1])
         {
            if(fpsp = this.panels.getPanel("fpsPanel") as FPSPanel)
            {
               highest = Number(remoteMSPFs[0]);
               fpsp.highest = highest;
               fpsp.averaging = highest;
               len = int(remoteMSPFs.length);
               for(i = 1; i < len; )
               {
                  if((fps = 1000 / remoteMSPFs[i]) > highest)
                  {
                     fps = highest;
                  }
                  fpsp.addCurrent(fps);
      ...
      {
         this.addLine(obj,priority,"C",false,skipSafe);
      }
      ...
      private function addLine(obj:*, priority:Number = 0, channel:String = null, isRepeating:Boolean = false, skipSafe:Boolean = false) : void
      {
         ...
         var txt:String = String(obj is XML || obj is XMLList ? obj.toXMLString() : obj);
         if(!channel || channel == " * ")
         {
            channel = "-";
         }
         ...
         if(!skipSafe)
         {
            txt = (txt = txt.replace(/</gim,"&lt;")).replace(/>/gim,"&gt;");
         }
         ...
         else
         {
            ...
            if(this.maxLines > 0)
            {
               if((off = this._lines.length - this.maxLines) > 0)
               {
      ...
      {
         return (this._viewingChannels.indexOf(" * ") >= 0 || this._viewingChannels.indexOf(line.c) >= 0 || this._filterText && this._viewingChannels.indexOf("~") >= 0 && line.text.toLowerCase().indexOf(this._filterText.toLowerCase()) >= 0) && (this._priority <= 0 || line.p >= this._priority);
      }
      ...
      {
         this.addLine(newLine,priority,"-",isRepeating);
      }
      
      public function log(... args) : void
      {
         this.addLine(this.joinArgs(args),2);
      }
      
      public function info(... args) : void
      {
         this.addLine(this.joinArgs(args),4);
      }
      
      public function debug(... args) : void
      {
         this.addLine(this.joinArgs(args),6);
      }
      
      public function warn(... args) : void
      {
         this.addLine(this.joinArgs(args),8);
      }
      
      public function error(... args) : void
      {
         this.addLine(this.joinArgs(args),10);
      }
      
      public function fatal(... args) : void
      {
         this.addLine(this.joinArgs(args),100);
      }
      
      public function logch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),2);
      }
      
      public function infoch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),4);
      }
      
      public function debugch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),6);
      }
      
      public function warnch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),8);
      }
      
      public function errorch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),10);
      }
      
      public function fatalch(channel:*, ... args) : void
      {
         this.ch(channel,this.joinArgs(args),100);
      }
      ...
      public function set filterText(str:String) : void
      {
         ...
         {
            this.clear("~");
            this._channels.splice(1,0,"~");
            this.addLine("Filtering [" + str + "]",10,"~");
            this.viewingChannels = ["~"];
         }
         else if(this.viewingChannel == "~")
         {
            this.viewingChannels = [" * "];
         }
      }
