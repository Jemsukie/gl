package com.luaye.console.view
{
   ...
   public class MainPanel extends AbstractPanel
   {
      ...
      public function MainPanel(m:Console, lines:Logs, channels:Array)
      {
         ...
         this._traceField.addEventListener("scroll",this.onTraceScroll,false,0,true);
         ...
         this._menuField.addEventListener("textLinkEvent",this.onMenuRollOver,false,0,true);
         ...
         this._commandField.type = "input";
         this._commandField.x = 40;
         this._commandField.height = 18;
         this._commandField.addEventListener("keyDown",this.commandKeyDown,false,0,true);
         this._commandField.addEventListener("keyUp",this.commandKeyUp,false,0,true);
         ...
         this._commandPrefx.type = "dynamic";
         ...
         this._commandPrefx.addEventListener("mouseDown",this.onCmdPrefMouseDown,false,0,true);
         this._commandPrefx.addEventListener("mouseMove",this.onCmdPrefRollOverOut,false,0,true);
         this._commandPrefx.addEventListener("rollOut",this.onCmdPrefRollOverOut,false,0,true);
         ...
         this._scrollbar.addEventListener("mouseDown",this.onScrollbarDown,false,0,true);
         ...
         this._scroller.addEventListener("mouseDown",this.onScrollerDown,false,0,true);
         ...
         addEventListener("link",this.linkHandler,false,0,true);
         addEventListener("addedToStage",this.stageAddedHandle,false,0,true);
         addEventListener("removedFromStage",this.stageRemovedHandle,false,0,true);
         master.cl.addEventListener("changedScope",this.onUpdateCommandLineScope,false,0,true);
      }
      ...
      {
         stage.addEventListener("keyUp",this.keyUpHandler,false,0,true);
         stage.addEventListener("keyDown",this.keyDownHandler,false,0,true);
      }
      
      private function stageRemovedHandle(e:Event = null) : void
      {
         stage.removeEventListener("keyUp",this.keyUpHandler);
         stage.removeEventListener("keyDown",this.keyDownHandler);
      }
      
      private function onCmdPrefRollOverOut(e:MouseEvent) : void
      {
         master.panels.tooltip(e.type == "mouseMove" ? TOOLTIPS["scope"] : "",this);
      }
      ...
      private function makeLine(line:Log) : String
      {
         ...
         if(master.prefixChannelNames && (master.viewingChannels.indexOf(" * ") >= 0 || master.viewingChannels.length > 1) && line.c != "-")
         {
      ...
      private function onScrollerDown(e:MouseEvent) : void
      {
         ...
         stage.addEventListener("mouseMove",this.onScrollerMove,false,0,true);
         stage.addEventListener("mouseUp",this.onScrollerUp,false,0,true);
      }
      ...
      private function onScrollerUp(e:MouseEvent) : void
      {
         ...
         stage.removeEventListener("mouseMove",this.onScrollerMove);
         stage.removeEventListener("mouseUp",this.onScrollerUp);
         this.onTraceScroll();
      }
      ...
      private function onScrollbarDown(e:MouseEvent) : void
      {
         ...
         this._scrolldelay = 0;
         this._scrollbar.addEventListener("enterFrame",this.onScrollBarFrame,false,0,true);
         stage.addEventListener("mouseUp",this.onScrollBarUp,false,0,true);
      }
      ...
      {
         this._scrollbar.removeEventListener("enterFrame",this.onScrollBarFrame);
         stage.removeEventListener("mouseUp",this.onScrollBarUp);
      }
      ...
      override public function set height(n:Number) : void
      {
         ...
         this._traceField.y = minimize ? 0 : 12;
         ...
         this._bottomLine.y = this._commandField.visible ? cmdy : n;
      ...
      public function getChannelsLink(limited:Boolean = false) : String
      {
         ...
         var str:* = "<chs>";
         var len:int = int(this._channels.length);
         if(limited && len > 7)
         {
            len = 7;
         }
         ...
         while(ci < len)
         {
            channel = String(this._channels[ci]);
      ...
      public function onMenuRollOver(e:TextEvent, src:AbstractPanel = null) : void
      {
         ...
         else if(txt == "channel_-")
         {
            txt = TOOLTIPS["defaultch"];
         }
         else if(txt == "channel_C")
         {
      ...
      private function linkHandler(e:TextEvent) : void
      {
         ...
         {
            master.runCommand("/remap 0|" + e.text.substring(6));
         }
         this._menuField.setSelection(0,0);
         e.stopPropagation();
      }
      ...
      public function onChannelPressed(chn:String) : void
      {
         ...
         if(this._shift && master.viewingChannel != " * " && chn != " * ")
         {
