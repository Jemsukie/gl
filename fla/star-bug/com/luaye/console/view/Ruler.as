package com.luaye.console.view
{
   ...
   public class Ruler extends Sprite
   {
      ...
      public function start(console:Console) : void
      {
         ...
         addEventListener("mouseMove",this.onMouseMove,false,0,true);
         this.onMouseMove();
      ...
      private function onMouseMove(e:MouseEvent = null) : void
      {
         ...
         this._pointer.blendMode = "invert";
         this._posTxt.text = "<s>" + mouseX + "," + mouseY + "</s>";
         this._posTxt.x = mouseX - this._posTxt.width - 12;
         this._posTxt.y = mouseY - this._posTxt.height - 12;
         if(this._posTxt.x < 0)
         {
            this._posTxt.x = mouseX + 12;
         }
         if(this._posTxt.y < 0)
         {
            this._posTxt.y = mouseY + 12;
         }
      }
      ...
      private function onMouseClick(e:MouseEvent) : void
      {
         ...
         else if(this._points.length == 1)
         {
            ...
            (txt = this.makeTxtField()).text = Utils.round(p.x,10) + "," + Utils.round(p.y,10);
            txt.x = p.x;
            txt.y = p.y - (ymin == p ? 14 : 0);
            addChild(txt);
            (txt = this.makeTxtField()).text = Utils.round(p2.x,10) + "," + Utils.round(p2.y,10);
            txt.x = p2.x;
            txt.y = p2.y - (ymin == p2 ? 14 : 0);
            addChild(txt);
            if(w > 40 || h > 25)
            {
               (txt = this.makeTxtField(43520)).text = Utils.round(mp.x,10) + "," + Utils.round(mp.y,10);
               txt.x = mp.x;
               txt.y = mp.y;
               addChild(txt);
            }
      ...
      {
         var format:TextFormat = new TextFormat("Arial",11,col,b,true,null,null,"right");
